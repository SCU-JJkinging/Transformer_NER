...
import os
import torch
from run_model.span_util import evaluate, train
from train_config.span_config import Config
from transformers import AdamW, get_linear_schedule_with_warmup
from data_processing.data_process import yeild_data
from model.model import BertSpanForNer
...
def main():
    ...
    label_list = ['x', 'pro', 'dis', 'sym', 'ite', 'bod', 'dru', 'mic', 'equ', 'dep', 'o']
    label2id = {label: i for i, label in enumerate(label_list)}
    id2label = {i: label for i, label in enumerate(label_list)}

    train_dataloader = yeild_data(config.train_file_data, label2id, model_type='span', is_train=True, ddp=False)
    val_dataloader = yeild_data(config.val_file_data, label2id, model_type='span', is_train=False, ddp=False)

    model = BertSpanForNer(config.pretrained_model_path,
                           len(label_list),
                           config.hidden_size,
                           config.loss_type,
                           config.with_start_label,
                           device).to(device)
