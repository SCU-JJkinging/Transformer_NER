...
def main():
    ...
    label_list = ['X', 'B-pro', 'I-pro', 'B-dis', 'I-dis', 'B-sym', 'I-sym', 'B-ite',
                  'I-ite', 'B-bod', 'I-bod', 'B-dru', 'I-dru', 'B-mic', 'I-mic',
                  'B-equ', 'I-equ', 'B-dep', 'I-dep', 'S-pro', 'S-dis', 'S-sym',
                  'S-ite', 'S-bod', 'S-dru', 'S-mic', 'S-equ', 'S-dep', 'O']

    label2id = {label: i for i, label in enumerate(label_list)}
    id2label = {i: label for i, label in enumerate(label_list)}

    train_dataloader = yeild_data(config.train_file_data, label2id, model_type='softmax', is_train=True, ddp=False)
    val_dataloader = yeild_data(config.val_file_data, label2id, model_type='softmax', is_train=False, ddp=False)

    model = BertSoftmaxForNer(config.pretrained_model_path,
                              len(label_list),
                              config.hidden_size,
                              config.loss_type,
                              device).to(device)
    ...
    for epoch in range(start_epoch, config.epochs + 1):
        ...
        with open('../result/softmax/metric.txt', 'a', encoding='utf-8') as fp:
            fp.write(
