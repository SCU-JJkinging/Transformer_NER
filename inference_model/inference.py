...
_, categories_size, _, id2categories = yeild_data(config.train_file_data, is_train=True, DDP=False)


def get_mapping(text):
    text_token = tokenizer.tokenize(text, max_length=config.max_len, add_special_tokens=True)
...
class NamedEntityRecognizer(object):
    ...
    def recognize(self, text, id2categories, model, threshold=0):
        ...
        encode_dict = tokenizer(text, return_offsets_mapping=True, padding='max_length', max_length=config.max_len,
...
def predict_to_file(in_file, out_file, categories_size, id2categories):
    ...
    """
    model = GlobalPointerNet(config.pretrained_model_path, categories_size, config.head_size, config.hidden_size).to(
        device)
    # model = nn.DataParallel(model.to(device), device_ids=gpus, output_device=gpus[0])
    model.load_state_dict(torch.load(config.target_dir))
